...
class OpenAIModel(LM):
    ...
    def load_model(self):
        ...
        # openai.api_base = "https://mtu.mtuopenai.xyz/v1/"
    ...
    def _generate(self, prompt, max_sequence_length=2048, max_output_length=128):
        ...
        if self.model_name == "ChatGPT":
            ...
            # Call API
            response = call_ChatGPT(message, prompt,temp=self.temp, max_len=max_sequence_length)
            # print('prompt:',prompt)
...
def call_ChatGPT(message, prompt, model_name="gpt-3.5-turbo", max_len=1024, temp=0.7, verbose=False):
    ...
    while not received:
        try:
            ...
            if error == openai.error.InvalidRequestError:
                ...
                # if prompt it too long or repeated, cut off it and try again
                prompt = prompt[:len(prompt)//2]
                message = [{"role": "user", "content": prompt}]
                assert False

            logging.error("API error: %s (%d). Waiting %dsec" % (error, num_rate_errors, 2))
